...
**Security headers come from two places, and you need both.** `_headers` is scoped `/*` but Cloudflare Pages applies it to **static assets only — never to a Pages Function's response**. So the `/api/*` Functions set the same four headers themselves, via the shared `json()` helper in `functions/_lib/http.mjs` that every `/api/*` return (success, 400, 404, 429, 500) already flows through. **Keep `_headers` and `SECURITY_HEADERS` in sync** — `tests/security-headers.test.mjs` parses `_headers` and fails on drift.

Verification trap, learned the hard way (2026-07-17): a `/api/*` path with **no Function behind it** (e.g. `/api/nonexistent-endpoint`) falls through to the static `404.html` and *does* carry the `_headers` set. Spot-checking such a path reports a false "4/4 on `/api/*`" — which is exactly how this file previously came to claim `_headers` "covers both static assets and `/api/*` Functions". **To check a Function, hit a real Function route** (`/api/audit`). Measured 2026-07-17 after the fix: 4/4 on `/`, `/audit`, `/contact`, `/blog/`, `/api/nonexistent-endpoint` **and** `/api/audit`.
